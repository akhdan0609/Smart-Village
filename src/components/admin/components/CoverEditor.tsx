...
import { RotateCcw, Send, Image as ImageIcon, Eye, Type, CheckCircle } from 'lucide-react';
...
export const COVER_META: Record<CoverKey, { label: string; desc: string; target: PageRoute; targetLabel: string }> = {
  beranda: {
...
export const CoverEditor: React.FC<CoverEditorProps> = ({ coverKey, onNavigate }) => {
    ...
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-extrabold text-slate-900">Sampul Halaman</h3>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-slate-100 pt-5">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 text-xs font-bold rounded-xl transition"
            >
              <RotateCcw className={`w-4 h-4 ${isResettingText === 'title' ? 'animate-spin' : ''}`} />
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 text-xs font-bold rounded-xl transition"
            >
              <RotateCcw className={`w-4 h-4 ${isResettingText === 'subtitle' ? 'animate-spin' : ''}`} />
